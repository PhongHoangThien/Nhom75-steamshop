...
import {FaFacebook, FaGoogle, FaTimes, FaEye, FaEyeSlash} from "react-icons/fa";
...
const Login = () => {
    ...
    const inputClass = "w-full bg-panelLight border border-border rounded-md px-4 py-3 focus:outline-none focus:border-primary text-text placeholder-textMuted transition [&::-ms-reveal]:hidden";
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                <div
                    className="bg-panel w-full max-w-5xl rounded-xl shadow-card overflow-hidden flex relative animate-fade-in border border-border">
                    <Link to="/"
                    ...
                    <div className="w-full md:w-[70%] p-8 md:p-12">
                        ...
                        <p className="text-textMuted text-sm mb-6">Đăng nhập để theo dõi đơn hàng, lưu danh sách sản
                            phẩm yêu thích và nhận nhiều ưu đãi hấp dẫn.</p>
                        {error &&
                            <p className="text-danger text-center text-sm mb-4 bg-red-500/10 p-2 rounded border border-danger">{error}</p>}
                        <form onSubmit={handleLogin} className="space-y-5">
                            <input type="text" className={inputClass} placeholder="Email hoặc tên đăng nhập"
                            ...
                            <div className="flex justify-between items-center text-sm">
                                <Link to="/forgot-password" className="text-primary hover:underline font-medium">Bạn
                                    quên mật khẩu?</Link>
                            </div>
                            <button disabled={isLoading}
                                    className="w-full bg-primary hover:bg-blue-600 text-white font-bold py-3 rounded-md transition duration-200 shadow-lg transform active:scale-95">
                                {isLoading ? "Đang xử lý..." : "Đăng nhập"}
                            </button>
                        </form>
                        <div className="my-6 flex items-center justify-center space-x-2">
                            <span className="h-px w-full bg-border"></span><span
                            className="text-textMuted text-sm whitespace-nowrap">Hoặc đăng nhập bằng</span><span
                            className="h-px w-full bg-border"></span>
                        </div>
